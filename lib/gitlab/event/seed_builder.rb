module Gitlab
  module Event
    class SeedBuilder
      class << self
        def create_created_group_group
          unless ::Event.find_by_author_id_and_action user.id, :created
          end
        end

        def pushed_project_push_summary
          event = ::Event.find_by_author_id_and_action user.id, :pushed
