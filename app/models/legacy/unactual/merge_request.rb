class Legacy::MergeRequest < LegacyDb
  #include Gitorious::Messaging::Publisher
  #include Watchable
  #include Gitorious::Search

  belongs_to :user
  belongs_to :source_repository, :class_name => Legacy::Repository
  belongs_to :target_repository, :class_name => Legacy::Repository

  has_many   :events, :as => :target, :dependent => :destroy
  has_many :messages, :as => :notifiable
  has_many :comments, :as => :target, :dependent => :destroy
  has_many :versions, :class_name => Legacy::MergeRequestVersion,
  ...
  named_scope :visible_by, Proc.new { |user|
    {
      ...
      :joins => [:target_repository => :project]
    }
  }
...
#   STATUS_VERIFYING = 4
  ...
  state_machine :status, :initial => :pending do
    state :pending, :value => Legacy::MergeRequest::STATUS_PENDING_ACCEPTANCE_OF_TERMS
    state :open, :value => Legacy::MergeRequest::STATUS_OPEN
    state :closed, :value => Legacy::MergeRequest::STATUS_CLOSED
